...
| `MIN_PASSWORD_LENGTH`     | `12`                                    | `src/passwords.js` (`getMinPasswordLength`)                                                               | Minimum accepted password length, enforced at register / change / reset. Can only **raise** the minimum above the built-in floor of `12` — a missing, non-numeric, or `<= 12` value falls back to `12`. When raised, a login with a now-too-short (but correct) password is paused: the user is redirected to `/password-upgrade` to set a longer one before a session is granted. |
