...
export const readKeyValues = (
  ...
  const rejected = rejectInvalidRole(role)
  if (rejected) return Promise.resolve(rejected)
  return readKeyValuesGeneric<TeamRoleKeyValueRow>(dbClient, SPEC, [team_uuid, role], owner)
}
...
export const searchKeyValues = (
  ...
  const rejected = rejectInvalidRole(role)
  if (rejected) return Promise.resolve(rejected)
  return searchKeyValuesGeneric<TeamRoleKeyValueRow>(dbClient, SPEC, [team_uuid, role], owner, pattern)
}
...
export const setKeyValue = (
  ...
  const rejected = rejectInvalidRole(role)
  if (rejected) return Promise.resolve(rejected)
  return upsertKeyValue(dbClient, SPEC.table, SPEC.subjectColumns, [team_uuid, role], owner, key, value)
}

/** Removes (team, role, owner, key); 404 if no such row. */
export const deleteKeyValue = (dbClient: DbClient, team_uuid: string, role: string, owner: Owner, key: string): Promise<Envelope<{}>> => {
  const rejected = rejectInvalidRole(role)
  if (rejected) return Promise.resolve(rejected)
