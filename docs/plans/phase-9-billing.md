...
- [x] `sql/billing_customers.sql` — one row per org with a Stripe customer ID (or equivalent). `org_uuid` PK (and FK to `organisations`, `ON DELETE CASCADE` — when an org is deleted the operator must handle the customer-side closeout separately, but the local record goes). Stores `provider`, `provider_customer_id`, `default_payment_method_id` (nullable), `tax_id` (nullable), `billing_email` (operator override, nullable), and `synced_email` (last effective email pushed to the provider — for drift detection). **Billing-email resolution:** `effective = billing_email ?? resolveBillingEmail(org)`, where `resolveBillingEmail` ranks members holding a verified primary email: billing-only → billing+owner → billing+admin → any-billing → owner (final fallback), tiebreak earliest-membership then email. The effective address is set on the Stripe customer at creation, re-synced immediately when the override changes, and reconciled **hourly** by `src/cron.ts` (patches the provider only on drift vs. `synced_email`).
...
- [x] `functions/api/db/auth/organisations/[org_uuid]/billing/` — the org-facing billing tree. Permission split (`org:billing:read` / `org:billing:write`) done in S3. Mutating endpoints verify the subscription belongs to the path org (returning 404, not 403, on mismatch to avoid existence leakage).
  ...
  - `email.ts` (POST, `org:billing:write`) — set/clear the `billing_email` override; re-resolves the effective address and syncs it to the provider (empty clears the override → falls back to the resolved billing contact).
...
- [~] **Operator endpoints** — under `functions/api/db/auth/admin/billing/`, behind the existing operator allowlist (`OPERATOR_USER_UUIDS`; the Phase 7 "TBD" is resolved). **Done (read-only):** `subscriptions.ts` (all subs across orgs) and `usage.ts` (rollup dashboard, optional `?org_uuid=`). **Handled Stripe-side, not in Puff:** _comp seats_ ride on a Stripe 100%-off coupon (Puff sees a normal `active` subscription — no special status); _manual invoices_ are raised in the Stripe dashboard. The webhook now attributes subscription-less invoices via a **customer fallback** (`invoice.customer` → `billing_customers` → org, `subscription_uuid` null), so operator-issued one-off invoices still land in Puff's records + `billing.invoice.*` audit events. _Refunds_ stay manual via the Stripe dashboard (see Out of scope).
