...
describe("team-role-keyvalues", () => {
  it("rejects unknown team roles with 400", async () => {
    const db = new FakeDb()
    expect((await readKeyValue(db.client, "team-1", "admin", orgOwner, "k")).status).toBe(400)
  ...
  it("accepts known team roles (lead/member)", async () => {
    ...
    expect((await readKeyValue(db.client, "team-1", "lead", orgOwner, "k")).success).toBe(true)
  ...
  it("setKeyValue carries (team_uuid, role) into the INSERT", async () => {
    ...
    await setKeyValue(db.client, "team-1", "lead", orgOwner, "max_concurrent", "10")
    const insert = db.calls.find((c) => c.text.startsWith("INSERT"))!
    expect(insert.values).toEqual(["team-1", "lead", "max_concurrent", "10", null, "org-1", null])
  })
})
