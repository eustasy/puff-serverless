...
import { deleteKeyValue, readKeyValue, readKeyValues, searchKeyValues, setKeyValue } from "../src/app-keyvalues.js"
...
describe("app-keyvalues readKeyValue", () => {
  ...
  it("returns 404 when no row matches", async () => {
    ...
    expect((await readKeyValue(db.client, "a-1", appOwner, "k")).status).toBe(404)
  })

  it("returns 500 on DB error", async () => {
    const db = new FakeDb()
    db.on(/SELECT kv_value FROM app_key_values/, pgError("08006"))
    expect((await readKeyValue(db.client, "a-1", appOwner, "k")).status).toBe(500)
  })
})
...
describe("app-keyvalues searchKeyValues", () => {
  ...
  it("rejects an empty pattern with 400", async () => {
    const db = new FakeDb()
    expect((await searchKeyValues(db.client, "a-1", appOwner, "  ")).status).toBe(400)
    expect(db.calls).toHaveLength(0)
  })
})
...
describe("app-keyvalues setKeyValue", () => {
  it("inserts a new row with the owner_app slot populated", async () => {
    ...
    const result = await setKeyValue(db.client, "a-1", appOwner, "theme", "default")
    ...
    expect(insert.values).toEqual(["a-1", "theme", "default", null, null, "a-1"])
  })
  ...
  it("allows an org to set a per-app entitlement row", async () => {
    ...
    const insert = db.calls.find((c) => c.text.startsWith("INSERT"))!
    expect(insert.values).toEqual(["a-1", "perm:export", "granted", null, "org-1", null])
  })
})
...
describe("app-keyvalues deleteKeyValue", () => {
  it("returns 404 when no row was deleted", async () => {
    ...
    expect((await deleteKeyValue(db.client, "a-1", appOwner, "k")).status).toBe(404)
  })

  it("returns 200 when a row was deleted", async () => {
    const db = new FakeDb()
    db.on(/DELETE FROM app_key_values/, { rowCount: 1 })
    expect((await deleteKeyValue(db.client, "a-1", appOwner, "k")).status).toBe(200)
  })
})
