...
describe("org-role-keyvalues", () => {
  it("rejects unknown roles with 400 before any query", async () => {
    const db = new FakeDb()
    expect(
      (await readKeyValue(db.client, "org-1", "ceo", orgOwner, "k")).status
    ).toBe(400)
    expect(db.calls).toHaveLength(0)

    expect(
      (await setKeyValue(db.client, "org-1", "ceo", orgOwner, "k", "v")).status
    ).toBe(400)

    expect(
      (await deleteKeyValue(db.client, "org-1", "ceo", orgOwner, "k")).status
    ).toBe(400)
  })
  ...
  it("accepts known org roles (owner/admin/member/billing)", async () => {
    ...
    expect(
      (await readKeyValue(db.client, "org-1", "admin", orgOwner, "k")).success
    ).toBe(true)
