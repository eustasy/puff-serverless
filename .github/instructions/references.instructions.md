---
applyTo: "**"
---

# References

## Frontend

* [HTMX Documentation](https://htmx.org/docs/)
* [HTMX References](https://htmx.org/reference/)

## Cloudflare Platform

* [Cloudflare Workers](https://developers.cloudflare.com/workers/)
* [Workers Static Assets](https://developers.cloudflare.com/workers/static-assets/)
* [Cloudflare Pages Functions](https://developers.cloudflare.com/pages/functions/)
* [Cloudflare Middleware](https://developers.cloudflare.com/pages/functions/middleware/)
* [Cloudflare Hyperdrive](https://developers.cloudflare.com/hyperdrive/)
* [Cloudflare Cron Triggers](https://developers.cloudflare.com/workers/configuration/cron-triggers/)
* [Wrangler CLI Documentation](https://developers.cloudflare.com/workers/wrangler/commands/)
* [Pages `_headers` reference](https://developers.cloudflare.com/pages/platform/headers/)
* [Pages `_redirects` reference](https://developers.cloudflare.com/pages/platform/redirects/)
* [Workers Custom Domains](https://developers.cloudflare.com/workers/configuration/routing/custom-domains/)

## Database

* [CockroachDB Documentation](https://www.cockroachlabs.com/docs/stable/)
* [CockroachDB SERIALIZABLE isolation & retries](https://www.cockroachlabs.com/docs/stable/transactions#transaction-retries)
* [node-postgres Documentation](https://node-postgres.com/)

## Authentication & Passwords

* [OTPLib Documentation](https://otplib.yeojz.dev/) — TOTP (RFC 6238) library.
* [RFC 6238 — TOTP](https://datatracker.ietf.org/doc/html/rfc6238)
* [HaveIBeenPwned Passwords API (k-anonymity)](https://haveibeenpwned.com/API/v2#SearchingPwnedPasswordsByRange)
* [zxcvbn — password strength estimator](https://github.com/dropbox/zxcvbn)
* [WebAuthn Level 3 spec](https://www.w3.org/TR/webauthn-3/)
* [SimpleWebAuthn server reference](https://simplewebauthn.dev/docs/packages/server)

## OAuth 2.1 / OIDC

* [OAuth 2.1 draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/)
* [RFC 7636 — PKCE](https://datatracker.ietf.org/doc/html/rfc7636)
* [OpenID Connect Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html)
* [OpenID Connect Discovery 1.0](https://openid.net/specs/openid-connect-discovery-1_0.html)
* [RFC 7517 — JSON Web Key](https://datatracker.ietf.org/doc/html/rfc7517)
* [RFC 7638 — JWK Thumbprint](https://datatracker.ietf.org/doc/html/rfc7638)
* [GitHub OAuth Apps](https://docs.github.com/en/apps/oauth-apps)
* [Google OAuth 2.0 for web server apps](https://developers.google.com/identity/protocols/oauth2/web-server)
* [Microsoft identity platform (Entra ID)](https://learn.microsoft.com/en-us/entra/identity-platform/v2-overview)

## Outbound mail

* [Mailtrap Send API](https://api-docs.mailtrap.io/docs/mailtrap-api-docs/bcf61cdc1547e-send-email-including-templates)

## Testing

* [Vitest](https://vitest.dev/)
