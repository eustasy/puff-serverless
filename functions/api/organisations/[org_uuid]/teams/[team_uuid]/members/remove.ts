...
export const onRequestPost: Handler<"org_uuid" | "team_uuid"> = async (context) => {
  ...
  if (!can(teamRoles, "team:members:remove") && !can(orgRoles, "org:teams:manage")) {
  ...
  const result = await removeTeamMember(context.data.dbClient!, String(context.params.team_uuid), user_uuid)
  if (!result.success) {
