...
/** Removes a team-subject KV row owned by this organisation. */
export const onRequestPost: Handler<"org_uuid" | "team_uuid"> = async (context) => {
  ...
  const result = await deleteKeyValue(context.data.dbClient!, team_uuid, { type: "org", org_uuid }, parsed.key)
  if (!result.success) {
