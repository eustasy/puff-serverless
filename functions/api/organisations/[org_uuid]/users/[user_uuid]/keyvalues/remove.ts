...
/** Removes a user-subject KV row owned by this organisation. */
export const onRequestPost: Handler<"org_uuid" | "user_uuid"> = async (context) => {
  ...
  const result = await deleteKeyValue(context.data.dbClient!, user_uuid, { type: "org", org_uuid }, parsed.key)
  if (!result.success) {
