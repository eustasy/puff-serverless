...
export const onRequestGet: Handler<"app_uuid" | "org_uuid"> = async (context) => {
  ...
  const rows: string[] = [`<tr><th scope="row">Mode</th><td>${escapeHtml(result.mode)}</td></tr>`]
  if (result.mode === "floating") {
    ...
    rows.push(`<tr><th scope="row">Assigned users</th><td>${result.assigned}</td></tr>`)
  }
  return htmlResponse(`<table class="licence-summary"><tbody>${rows.join("")}</tbody></table>`)
}
