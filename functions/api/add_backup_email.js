...
export async function onRequestPost(context) {
  ...
  if (!backup_email || typeof backup_email !== "string" || !backup_email.includes("@")) {
    return new Response(
  ...
  try {
    ...
    if (error.message && error.message.includes("UNIQUE constraint failed: emails.user_uuid, emails.email_address")) {
        return new Response(
            JSON.stringify({ error: "This email address is already associated with your account." }),
            { status: 409, headers: { "Content-Type": "application/json" } }
          );
    }
    return new Response(
      JSON.stringify({ error: "Failed to add backup email due to a server error." }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
}
