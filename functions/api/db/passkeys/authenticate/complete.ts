...
export const onRequestPost: Handler = async (context) => {
  ...
  if (!challenge) {
    return new Response(
      '<p class="result-negative">Authentication session expired. Please try again.</p>',
  ...
  if (!credentialId || typeof credentialId !== "string") {
    return new Response(
      '<p class="result-negative">Missing credential ID.</p>',
      { status: 400, headers: { "Content-Type": "text/html" } }
    )
  }
  ...
  if (!passkeyResult.success) {
    return new Response(
      '<p class="result-negative">Passkey not found. Please sign in with your password.</p>',
      { status: 400, headers: { "Content-Type": "text/html" } }
    )
  }
  ...
  if (passkey.user_uuid !== user_uuid) {
    return new Response(
      '<p class="result-negative">Passkey does not match account.</p>',
  ...
  if (!verification.verified) {
    return new Response(
      '<p class="result-negative">Passkey could not be verified.</p>',
  ...
  if (!sessionResult.success) {
    console.error(
      "Error creating session after passkey auth:",
      sessionResult.error
    )
    return new Response(
      '<p class="result-negative">Could not create session. Please try again.</p>',
