...
import { savePasskey, listPasskeys, getRpConfig } from "../../../../../../src/passkeys.js"
...
export const onRequestPost: Handler = async (context) => {
  ...
  if (!challenge) {
    return new Response('<p class="result-negative">Registration session expired. Please try again.</p>', {
      status: 400,
  ...
  const tokenResult = await consumeToken(dbClient, challenge, "webauthn_registration_challenge")
  ...
  if (!verification.verified || !verification.registrationInfo) {
    return new Response('<p class="result-negative">Passkey registration could not be verified.</p>', {
      status: 400,
      headers: { "Content-Type": "text/html" },
    })
  }
  ...
  if (saveResult.error || !saveResult.success) {
    return new Response('<p class="result-negative">Failed to save passkey. Please try again.</p>', {
      status: 500,
      headers: { "Content-Type": "text/html" },
    })
  }
  ...
  const clearCookie = ["webauthn_challenge_token=", "HttpOnly", "Path=/", `SameSite=${sameSite}`, "Max-Age=0"]
  if (secure) clearCookie.push("Secure")

  return new Response('<p class="result-positive">Passkey registered successfully.</p>', {
    status: 200,
    headers: {
