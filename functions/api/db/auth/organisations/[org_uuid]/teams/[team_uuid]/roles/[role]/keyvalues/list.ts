import {
  readKeyValues,
  searchKeyValues,
} from "../../../../../../../../../../../src/team-role-keyvalues.js"
...
export const onRequestGet: Handler<"org_uuid" | "team_uuid" | "role"> = async (
  context
) => {
  const orgRoles = context.data.orgRoles ?? []
  const teamRoles = context.data.teamRoles ?? []
  if (
    !can(orgRoles, "org:keyvalues:read") &&
    !can(teamRoles, "team:keyvalues:read")
  ) {
  ...
  const canWrite =
    can(orgRoles, "org:keyvalues:write") ||
    can(teamRoles, "team:keyvalues:write")
  return renderKeyValueTable(result.pairs, {
    removeBase: `/api/db/auth/organisations/${encodeURIComponent(org_uuid)}/teams/${encodeURIComponent(team_uuid)}/roles/${encodeURIComponent(role)}/keyvalues/remove`,
