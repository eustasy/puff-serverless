...
export const onRequestPost: Handler<"org_uuid"> = async (context) => {
  ...
  const result = await enableOrganisation(context.data.dbClient!, String(context.params.org_uuid))
  if (!result.success) {
    return resultNegative(result.error ? "Could not enable the organisation." : "Organisation not found.", result.status)
  }
