  ...
  try {
    ...
    if (currentPrimaryRecord) {
      ...
      if (demoteResult.meta.changes > 0) {
        ...
        console.warn(
          `Could not demote old primary email ${currentPrimaryRecord.email_address} for user ${user_uuid} or it was already demoted.`
        )
      }
    } else {
      // No current primary found, this is unusual but not necessarily an error for promoting a new one.
      console.warn(
        `No current primary email found for user ${user_uuid} when trying to change primary to ${new_primary_email}.`
      )
    ...
    if (promoteResult.meta.changes > 0) {
      ...
      if (oldPrimaryEmailDemoted && currentPrimaryRecord) {
        await context.env.DATABASE.prepare(
          ...
          .run()
        console.error(
          `Failed to promote ${new_primary_email} for user ${user_uuid}. Attempted to restore old primary ${currentPrimaryRecord.email_address}.`
        )
        return new Response(
          JSON.stringify({
            error:
              "Failed to promote new primary email. Old primary status potentially restored.",
          }),
          { status: 500, headers: { "Content-Type": "application/json" } }
        )
      }
      console.error(
        `Failed to promote ${new_primary_email} for user ${user_uuid} and no old primary to restore or demotion failed.`
      )
      return new Response(
        JSON.stringify({
