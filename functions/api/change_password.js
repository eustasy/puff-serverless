...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!secretRecord || !secretRecord.secret_value) {
      ...
      return new Response(
        JSON.stringify({ error: "Could not verify current password. User record issue." }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    const { hash: newHash, salt: newSalt } = await puff_hashing_password(new_password);
    ...
    if (updatePasswordStmt.meta.changes === 0) {
        console.error(`Failed to update password for user_uuid (no rows changed): ${user_uuid}`);
        return new Response(JSON.stringify({ error: "Failed to update password. Please try again." }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    return new Response(
      JSON.stringify({ error: "Failed to change password due to a server error." }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
}
