...
//
// This sits beside set.ts in the unauthenticated functions/api/password/
...
import { consumeToken, createLoginToken, readToken } from "../../../src/tokens.js"
...
export const onRequestPost: Handler = async (context) => {
  ...
  const upgradeToken = await getCookie(context.request.headers.get("Cookie"), "password_upgrade_token")
  if (!upgradeToken) {
  ...
  try {
    ...
    if (!(await passwordRequirements(new_password, passwordConfig(context.env)))) {
    ...
    if (!pending || pending.token_type !== "password_upgrade" || pending.is_used || new Date(pending.expires_at) < new Date()) {
      return new Response('<p class="result-negative">Your login session has expired. Please log in again.</p>', {
        status: 400,
        headers: { "Content-Type": "text/html" },
      })
    ...
    const tokenResult = await consumeToken(dbClient, upgradeToken, "password_upgrade")
    ...
    if (updateResult.error || !updateResult.success) {
      console.error(`Failed to update password during upgrade for user_uuid ${user_uuid}: ${updateResult.message}`)
      return new Response('<p class="result-negative">Failed to update password. Please log in again.</p>', {
        status: 500,
        headers: { "Content-Type": "text/html" },
      })
    }
    ...
    const clearUpgrade = ["password_upgrade_token=", "HttpOnly", "Path=/", `SameSite=${sameSite}`, "Max-Age=0"]
    if (secure) clearUpgrade.push("Secure")

    const twoFactorResult = await has2fa(dbClient, user_uuid)
    if (twoFactorResult.error) {
      console.error("Error checking 2FA status during password upgrade:", twoFactorResult.message)
      return new Response('<p class="result-negative">Could not complete login. Please log in again.</p>', {
        status: 500,
        headers: { "Content-Type": "text/html" },
      })
    }
    ...
    if (twoFactorResult.enabled) {
      ...
      if (loginToken.error) {
        console.error("Error creating TOTP token after password upgrade:", loginToken.message)
        return new Response('<p class="result-negative">Could not start 2FA. Please log in again.</p>', {
          status: 500,
          headers: { "Content-Type": "text/html" },
        })
      }
    ...
    if (!sessionResult.success) {
      console.error("Error creating session after password upgrade:", sessionResult.error)
      return new Response('<p class="result-negative">Could not complete login. Please log in again.</p>', {
        status: 500,
        headers: { "Content-Type": "text/html" },
      })
    ...
    return new Response('<p class="result-negative">Failed to upgrade password due to a server error.</p>', {
      status: 500,
      headers: { "Content-Type": "text/html" },
    })
  }
