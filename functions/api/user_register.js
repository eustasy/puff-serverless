...
export async function onRequest(context) {
  ...
  const insert_secret = await context.env.DATABASE.prepare(
    "INSERT INTO secrets (user_uuid, secret_type, secret_value, secret_created_at) VALUES (?1, ?2)"
  )
    .bind(uuid, "SHA-384", secret_value, now)
    .run()
