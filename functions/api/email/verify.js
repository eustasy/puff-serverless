  ...
  try {
    ...
    if (token_type === 'email_verification') {
      ...
      if (!emailDetails) {
        ...
        return new Response(
          JSON.stringify({
            error: "Associated backup email record not found or is primary. Please try adding the backup email again.",
          }),
          { status: 400, headers: { "Content-Type": "application/json" } }
        );
      }
      ...
      console.error(`Unknown token type: ${token_type} for token: ${tokenValue}`);
      return new Response(
    ...
    if (updateEmailResult.rowCount === 0) {
      ...
      return new Response(
        JSON.stringify({
          error: "Failed to verify email. The email may not exist or conditions not met (e.g., backup email is primary).",
        }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    const successMessage = token_type === 'email_verification' 
    ...
    if (tokenValue && client && client._connected && tokenRecord && tokenRecord.token_type) {
      try {
        await client.query(
          "UPDATE tokens SET is_used = TRUE WHERE token_value = $1 AND token_type = $2",
          [tokenValue, tokenRecord.token_type]
