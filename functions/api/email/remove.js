...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (emailCheckResult.rows[0].is_primary) {
        return new Response(
            JSON.stringify({ error: "Cannot remove the primary email address. Please set another email as primary first." }),
            { status: 400, headers: { "Content-Type": "application/json" } }
        );
    }
    ...
    if (deleteResult.rowCount === 0) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to remove email. It might have been already removed or an issue occurred." }),
        { status: 404, headers: { "Content-Type": "application/json" } }
      );
    }
