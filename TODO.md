...
- [x] ID tokens (and access tokens) are signed JWTs via Web Crypto.
  ...
  - Rotation: **manual operator action**, not a cron job. Generator + step-by-step in `scripts/generate-oauth-key.mjs`; full procedure in `ARCHITECTURE.md → OAuth signing-key rotation`. Overlap window keeps in-flight JWTs valid until they expire (~1h); the operator clears the previous-public binding after that.
