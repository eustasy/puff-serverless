...
- [x] **.html auth handling.** `functions/_middleware.ts` (root middleware) redirects unauthenticated requests for `/account` and `/logout` to `/login` via 302 when no `session_token` cookie is present. Cookie-presence only — no DB query; the real auth gate remains in the API-layer middleware. `/2fa` and `/password-upgrade` are intentionally excluded (mid-login flow pages use different token cookies). Requires `assets.run_worker_first: ["/account", "/logout"]` in `wrangler.jsonc` — static assets are served before the Worker by default, so the middleware would otherwise never run for those paths — plus `assets.binding: "ASSETS"` so `context.next()` can fall through to `env.ASSETS.fetch()` and serve the HTML for authenticated requests. **This does not work. Static assets do not run middleware.**
- [ ] Transactions are always run in serializable isolation level
